...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, code, status, deletion_requested_at FROM tickets WHERE id = $1 FOR UPDATE`,
      [id]
    );
    if (rows.length === 0) {
    ...
    if (!rows[0].deletion_requested_at) {
      await client.query("ROLLBACK");
      return jsonError("El ticket no tiene una solicitud de eliminación pendiente");
    }
    ...
    if (actorId) {
      await client.query(
        `INSERT INTO status_history (entity_type, entity_id, from_status, to_status, changed_by, reason)
         VALUES ('ticket', $1, $2, 'deleted', $3, 'Eliminación aprobada por administrador')`,
    ...
    await client.query("ROLLBACK");
    return jsonError("No se pudo eliminar el ticket", 500, String(err));
  } finally {
    client.release();
  }
}
